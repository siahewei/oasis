...
//
//    private final IStickyHeaderDecoration mAdapter;
//    private final IOrientationProvider mOrientationProvider;
//    private final IStickyHeaderProvider mHeaderProvider;
//    private final VmStickyDecoration.DimensCalculator mDimensionCalculator;
//
//    /**
...
//
//    public HeaderPositionCalculator(IStickyHeaderDecoration adapter, IStickyHeaderProvider headerProvider,
//                                    IOrientationProvider orientationProvider, VmStickyDecoration.DimensCalculator dimensionCalculator) {
//        mAdapter = adapter;
...
//        }
//
//        return offset <= margin && mAdapter.hasStickHeader(position);
//    }
//
//    /**
//     * 判断下一个itemview是否有新的header
...
//        if (indexOutOfBounds(parent.getAdapter(), position)) {
//            return false;
//        }
//
//        // 只有当前已经有了viewHeader,这种判断才有意义
//        if (!mAdapter.hasStickHeader(position)) {
//            return false;
//        }
//
//        int headerId = mAdapter.getHeaderViewTag(position, parent);
...
//            nextItemHeaderId = mAdapter.getHeaderViewTag(nextItemPosition, parent);
//        }
...
//            int viewHeaderTag = mAdapter.getHeaderViewTag(firstViewUnderHeaderPosition, recyclerView);
//            View secondHeader = mHeaderProvider.getHeader(mAdapter, recyclerView, firstViewUnderHeaderPosition, viewHeaderTag);
...
//        }
//
//        headerMargins.set(translationX, translationY, translationX + header.getWidth() - rightMargin,
...
//        if (firstViewUnderHeaderPosition > 0 && hasNewHeader(recyclerView, firstViewUnderHeaderPosition, isReverseLayout)) {
//            int headerViewTag = mAdapter.getHeaderViewTag(firstViewUnderHeaderPosition, recyclerView);
//            View nextHeader = mHeaderProvider.getHeader(mAdapter, recyclerView, firstViewUnderHeaderPosition, headerViewTag);
...
//        int headerViewTag = mAdapter.getHeaderViewTag(adapterPosition, parent);
//        if (adapterPosition == RecyclerView.NO_POSITION || mHeaderProvider.getHeader(mAdapter, parent, adapterPosition, headerViewTag) != header) {
//            return false;
//        }
//
//        if (orientation == LinearLayoutManager.VERTICAL) {
